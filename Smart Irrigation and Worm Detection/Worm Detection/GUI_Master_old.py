...
root.title("Smart Irrigation and Pesticide Management using worm Detection")
...
#
lbl = tk.Label(root, text="Smart Irrigation and Pesticide Management using Worm Detection ", font=('times', 20,' bold '), height=1, width=85,bg="violet Red",fg="Black")
lbl.place(x=0, y=0)
...
def sendMsg(msg):
    ...
    my_data = {
        ...
        'message': f'Dear User, \n "{msg}" Worms Detected on the crops Turning Motor On..',
    ...
    # create a dictionary
    headers = {
        'authorization': '6EsJ1ZIHQyjBhDNSar9X8b20LcRPfUli7tCdon3GukMVFAvTz5sLYVPXjGdbaylQJxrq8D79kINg2c4v',
...
#    global fn
    ...
    if fn!="":
        ...
        if Nutrient == 0:
             Cd="Groundnut Prelims"
             sendMsg(Cd)
        elif Nutrient == 1:
            Cd="Larva-Spodopteralitura"
            sendMsg(Cd)
        elif Nutrient == 2:
            Cd="Normal Detected"
...
def openimage():
    ...
    global fn
    fileName = askopenfilename(initialdir='D:/project/warm Detection', title='Select image for Aanalysis ',
                               filetypes=[("all files", "*.*")])
    IMAGE_SIZE=200
    imgpath = fileName
    fn = fileName
